...
## Basic usage of `tigris` (RCensusPkg::)

:::task
Define an output directory for acquiring downloaded shapefiles from the Census Bureau.
:::
```{r}
...
Download from the US Census Bureau the shapefile of US states and return the resulting simple feature object(sf):
...
#| code-fold: true
#| fig-cap: |
#|   Figure 5.1: Default US states data obtained with RcensusPkg::tiger_states_sf
...
nm_counties_plot <- RspatialPkg::get_geom_sf(
  sf = nm_counties_sf
)
nm_counties_plot
```
...
la_tracts_plot <- RspatialPkg::get_geom_sf(sf = la_tracts_sf)
la_tracts_plot
```
...
los_alamos_area_water_plot <- RspatialPkg::get_geom_sf(
  sf = los_alamos_area_water_sf
)
los_alamos_area_water_plot 
```

### Understanding tigris(RcensusPkg) and simple features
...
dc_landmarks_plot <- RspatialPkg::get_geom_sf(
  sf = dc_landmarks_sf,
  sf_shape = 1,
  sf_size = 4
)
...
dc_roads_plot <- RspatialPkg::get_geom_sf(sf = dc_roads_sf)
dc_roads_plot
```
...
::: task
Retrieve the default crs (i.e. NAD83) of Florida counties simple feature using `sf::st_crs()`.
:::
...
florida_crs_gt <- RplotterPkg::create_table(
  x = florida_counties_suggest_df,
...
### Plotting with `ggplot2::coord_sf()`
...
florida_counties_3087_plot <- RspatialPkg::get_geom_sf(florida_counties_sf) +
    ggplot2::coord_sf(crs = 3087, datum = 3087)
florida_counties_3087_plot
```
...
us_states_plot <- RspatialPkg::get_geom_sf(
  sf = us_states_sf,
...
us_states_crs_plot <- RspatialPkg::get_geom_sf(
  sf = us_states_sf,
  panel_border_color = NA,
  hide_x_tics = T,
  hide_y_tics = T
...
texas_places_plot <- RspatialPkg::get_geom_sf(
  sf = texas_sf,
  sf_fill = "yellow",
  sf_stroke = 2.0,
  hide_x_tics = T,
...
lee_county_plot <- RspatialPkg::get_geom_sf(
  sf = lee_county_sf,
  sf_fill = "#FF8C19"
)
lee_county_plot
...
sanibel_plot <-  RspatialPkg::get_geom_sf(
 sf = sf::st_as_sf(lee_singlepart_sf[2, ]),
 sf_fill = "#FF8C19"
) + 
 coord_sf(crs = 3087, datum = 3087)
sanibel_plot
#mapview(lee_singlepart_sf[2, ])
```

### Conclusion
This concludes the notes on Chapter 5 "Census geographic data and applications in R".
