...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.1: Median age for US counties, acs/acs5, 2020

county_med_age_gt <- RplotterPkg::create_table(
  x = county_med_age_dt[1:6,],
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.2: The youngest counties in the US by median age,acs/acs5, 2020

county_med_age_young_to_old_gt <- RplotterPkg::create_table(
  x = county_med_age_young_to_old_dt[1:6,],
  container_width_px = 400
)
county_med_age_young_to_old_gt
```
...
Order the "estimate" column of `county_med_age_dt` via descending direction:
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.3: The oldest counties in the US by median age,acs/acs5, 2020

county_med_age_old_to_young_gt <- RplotterPkg::create_table(
  x = county_med_age_old_to_young_dt[1:6,],
  container_width_px = 400
)
county_med_age_old_to_young_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.4: Counties with a median age of 50 or above, acs/acs5, 2020

county_med_age_gte_50_gt <- RplotterPkg::create_table(
  x = county_med_age_gte_50_dt[1:6,],
  container_width_px = 400
)
county_med_age_gte_50_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.5: Separate columns for county and state,acs/acs5,2020
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.6: Race and ethnicity in Arizonia,acs/acs5,2020

estimate_long_gt <- RplotterPkg::create_table(
  x = estimate_long_dt[1:6,],
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.7: Race and ethnicity in Arizonia as percentages,acs/acs5,2020

estimate_percent_long_gt <- RplotterPkg::create_table(
  x = estimate_percent_long_dt[1:6,],
  container_width_px = 400
)
estimate_percent_long_gt
```
...
```{r}
estimate_percent_grouped_dt <- estimate_percent_long_dt[, .SD[percent == max(percent)], by = NAME]
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.8: Largest group by county in Arizona, acs/acs5, 2020

estimate_percent_grouped_gt <- RplotterPkg::create_table(
  x = estimate_percent_grouped_dt,
  container_width_px = 400
)
estimate_percent_grouped_gt
```
...
```{r}
estimate_percent_median_dt <- estimate_percent_long_dt[, .(median_pct = stats::median(percent)), by = variable] %>% # compute the median percent for each group
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.9: Median percentage by group in Arizona, acs/acs5, 2020

estimate_percent_median_gt <- RplotterPkg::create_table(
  x = estimate_percent_median_dt,
  container_width_px = 400
)
estimate_percent_median_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.10: Table/Group B19001 for counties in Minnesota,acs/acs5, 2016

mn_hh_income_gt <- RplotterPkg::create_table(
  x = mn_hh_income_dt[1:6,],
  container_width_px = 400
)
mn_hh_income_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.11: Recoded household income categories,acs/acs5, 2016

mm_hh_income_recode_gt <- RplotterPkg::create_table(
  x = mm_hh_income_recode_dt[1:12,],
  container_width_px = 400
)
mm_hh_income_recode_gt
```
...
```{r}
mm_hh_income_recode_grouped_dt <- mm_hh_income_recode_dt[, .(estimate = sum(estimate)), by = c("GEOID", "incgroup")] 
```

```{r}
#| code-fold: true
#| tbl-cap: |
#|   Table 3.12: Grouped sums by income bands,acs/acs5, 2016

mm_hh_income_recode_grouped_gt <- RplotterPkg::create_table(
  x = mm_hh_income_recode_grouped_dt[1:6,],
  container_width_px = 400
)
mm_hh_income_recode_grouped_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.13: Age table/group for Oglala Lakota County, SD,acs/acs5,2020

oglala_lakota_age_2020_gt <- RplotterPkg::create_table(
  x = oglala_lakota_age_2020_dt[1:6,],
  container_width_px = 400
)
oglala_lakota_age_2020_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.14: Age table/group for Oglala Lakota County, SD (then named Shannon County), 2010

oglala_lakota_age_2010_gt <- RplotterPkg::create_table(
  x = oglala_lakota_age_2010_dt[1:6,],
  container_width_px = 400
)
oglala_lakota_age_2010_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.15: DP02_0068 variable names available from acs/acs1/profile, 2019

DP02_0068_2019_gt <- RplotterPkg::create_table(
  x = DP02_0068_2019_dt,
  container_width_px = 500
)
DP02_0068_2019_gt
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.16: DP02_0068 variable names available from acs/acs1/profile, 2018

DP02_0068_2018_gt <- RplotterPkg::create_table(
  x = DP02_0068_2018_dt,
  container_width_px = 500
)
DP02_0068_2018_gt
...
#| code-fold: true
#| tbl-cap: |
#|   Variables available that addresses median household income,acs/acs5/cprofile,2020

hh_income_vars_gt <- RplotterPkg::create_table(
  x = hh_income_vars_dt,
  container_width_px = 500
)
hh_income_vars_gt
...
alaska_hh_incomes_dt <- get_vintage_data(
  dataset = "acs/acs5/cprofile",
  vintage = 2020,
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.17: Comparative income data among Alaska counties, acs/acs5/cprofile, 2020

alaska_hh_incomes_long_gt <- RplotterPkg::create_table(
  x = alaska_hh_incomes_long_dt[1:6,],
  container_width_px = 400
)
alaska_hh_incomes_long_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   'B15002' variables of interest, acs/acs1, 2019

vars_B15002_gt <- RplotterPkg::create_table(
  x = vars_B15002_dt,
  container_width_px = 500
)
vars_B15002_gt
...
Both `college_dt` and `college_alt_dt` are identical.
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.18: Educational attainment over time for Colorado counties, acs/acs1,2010 to 2019

college_by_year_estimate_long_gt <- RplotterPkg::create_table(
  x = college_by_year_estimate_long_dt[1:6,],
  container_width_px = 500
)
college_by_year_estimate_long_gt
```
...
#| code-fold: true
#| tbl-cap: |
#|   Table 3.19: Percent college by year in Colorado counties,acs/acs1

percent_college_by_year_gt <- RplotterPkg::create_table(
  x = percent_college_by_year_dt,
  container_width_px = 650
)
percent_college_by_year_gt
```
